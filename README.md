...
| <a name="input_universe"></a> [universe](#input\_universe) | Dedicated/sovereign universe settings. Null = public googleapis.com universe. When set, the caller must also set universe\_domain on the google/google-beta providers. | <pre>object({<br/>    prefix = string # Universe prefix, e.g. the '<prefix>' in '<prefix>-system.iam.gserviceaccount.com'<br/>  })</pre> | `null` | no |
